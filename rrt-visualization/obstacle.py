...
class Obstacle(Drawable):
    def __init__(self, center: Vector2, width: float = 50) -> None:
        super().__init__()
        self.rect = Rect(center.x - width / 2, center.y - width / 2, width, width)  #Rect ne ?
    ...
    def draw(self, screen: Surface) -> None:
        rect(screen, (0, 0, 0), self.rect)   # rect ne ? 

    def collided(self, point: Vector2) -> bool:
        return self.rect.collidepoint(point.x, point.y) # collide point ne ? 
